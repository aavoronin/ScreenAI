...
class ChunkHtmlHelper:
    ...
    def _parse_time_ago_from_text(vacancy_text):
        """
        Parse time phrases from vacancy text to find when it was posted.
        ...
        patterns = [
            # Reposted X time ago
            (r'reposted\s+(\d+)\s+(hour|hours|day|days|week|weeks|month|months|year|years)\s+ago', True),
            # Posted X time ago
            (r'posted\s+(\d+)\s+(hour|hours|day|days|week|weeks|month|months|year|years)\s+ago', False),
            # Just X time ago (without posted/reposted)
            (r'(\d+)\s+(hour|hours|day|days|week|weeks|month|months|year|years)\s+ago', False),
        ]

        for pattern, is_reposted in patterns:
            match = re.search(pattern, vacancy_text, re.IGNORECASE)
            if match:
                ...
                return delta, is_reposted

        return None

    @staticmethod
    def _calculate_publication_date(saved_date_str, vacancy_text):
        """
        ...
        try:
            ...
            if time_delta_info:
                ...
                return publication_date.strftime("%Y-%m-%d"), is_reposted
            else:
                # If no time phrase found, just return the saved date
                return saved_date.strftime("%Y-%m-%d"), False

        except Exception as e:
            print(f"️ Error calculating publication date: {e}")
            return "", False
...
"""]
        ...
        html_parts.append(
            ...
            '    <th style="width: 22%;">VacancyTitle</th>\n'
            '    <th style="width: 11%;">Date</th>\n'
            '    <th style="width: 11%;">Score</th>\n'
            '    <th style="width: 11%;">Score Percentile</th>\n'
            '    <th style="width: 11%;">VacancyId</th>\n'
            '    <th style="width: 11%;">Salary</th>\n'
