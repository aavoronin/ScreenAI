...
class LinkedInNavigator(BaseNavigator):
    ...
    def _execute_linkedin_automation(self):
        ...
        while not general_abort:
            ...
            if self.parser.refresh_error:
                print("⚠️ 'Something went wrong' detected. Looking for reload button...")
                reload_btn = self.parser.find_reload_button()
                if reload_btn:
                    print("🔄 Reload button found. Clicking and waiting 30 seconds...")
                    self.click_bbox_center(reload_btn['bbox'])
                    time.sleep(30)
                    print("🔄 Reparsing screen after reload...")
                    screenshot = self._grab_screenshot()
                    self.parser.parse_screen(screenshot)
                    if self.parser.refresh_error:
                        print("❌ 'Something went wrong' still present after reload. Aborting loop.")
                        break
                    else:
                        print("✅ Screen recovered after reload. Continuing loop.")
                else:
                    print("❌ Reload button not found. Aborting loop.")
                    break
            ...
            if next_buttons:
                ...
                print(f"🔻 Scroll down (triangle_down) detected. Clicking {self.MAX_SCROLL_DOWNS} times...")
                pyautogui.press('esc')
                pyautogui.press('esc')
                time.sleep(0.3)
